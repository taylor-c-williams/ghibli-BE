...
app.get('/films', async(req, res) => {
  try {
    ...
    films.id, 
      ...
      FROM films
      JOIN creators
      ON films.creators_id = creators.id
      `);    
    res.json(data.rows);
  } catch(e) {    
...
app.get('/films/creators', async(req, res) => {
  try {
    const data = await client.query('SELECT * from creators');    
...
app.get('/films/:id', async(req, res) => {
  try {
    ...
    films.id, 
      ...
      JOIN creators
      ON films.creators_id = creators.id 
      where id= creators.$1`, [req.params.id]);
